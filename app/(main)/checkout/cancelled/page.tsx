...
export default async function CheckoutCancelledPage({ searchParams }: Props) {
  const session = await requireUser({ next: "/checkout/cancelled" });
  ...
  return (
    <PageShell narrow>
      <PageIntro
        ...
        description="No charge was completed. You can return to your cart and try again when ready."
      />
      {owned ? (
        <p className="mt-6 text-sm text-muted-foreground">
          Pending order {owned.orderNumber} remains unpaid and will not start fulfillment.
        </p>
      ) : null}
